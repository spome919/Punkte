...
async def join_channel(event):
    try:
        await sython1(JoinChannelRequest("@Haider_1h1"))
...
async def join_channel(event):
    try:
        await sython1(JoinChannelRequest("@G_lRR"))
    except BaseException:
        pass
...
async def join_channel(event):
    try:
        await sython1(JoinChannelRequest("@uus_o"))
    except BaseException:
        pass
...
async def join_channel(event):
    try:
        await sython1(JoinChannelRequest("@BB70_B"))
    except BaseException:
        pass
    ...
    if sender.id == ownerhson_id:
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await sython1.send_message(event.chat_id, f"تم الانتهاء من التجميع | 𝗔𝗥𝗕𝗘𝗥")
        ...
        await sython1.send_message(event.chat_id, "تم الانتهاء من التجميع | 𝗔𝗥𝗕𝗘𝗥")
    ...
    while True:
        try:
            ...
            if sender.id == ownerhson_id:
                ...
                joinu = await sython1(JoinChannelRequest('saythonh'))
                channel_entity = await sython1.get_entity(pot)
                await sython1.send_message(pot, '**جاري بدأ عملية التجميع بواسطة العرب**')
                await sython1.send_message(pot, '/start')
                await asyncio.sleep(2)
                msg0 = await sython1.get_messages(pot, limit=1)
                await msg0[0].click(2)
